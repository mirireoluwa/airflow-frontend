...
export function OverdueTaskIndicator() {
  ...
  const overdueTasks = state.currentUser.role === 'employee' 
    ? getOverdueTasksForUser(state.currentUser.id, state.tasks, state.users)
    : getOverdueTasksForManager(state.currentUser.id, state.tasks, state.users);
    ...
    <div className="bg-white border border-gray-200 rounded-lg p-6">
      ...
      <div className="space-y-3">
        {overdueTasks.slice(0, 3).map((overdue) => (
          <div 
            key={overdue.task.id}
            className={`p-3 rounded-xl border ${getOverdueTaskColor(overdue.daysOverdue)}`}
          >
